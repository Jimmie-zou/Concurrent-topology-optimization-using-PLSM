function [dCH11,dCH12,dCH13,dCH22,dCH23,dCH33,KE,A0,CH,den] = FEACalculation_CM(Phi0,nelx,nely,ew,eh,EW,EH,E,v)
% start sub-function Fem_can
% close all;
% x = x0 = reshape(Phi0(1:(nely+1)*(nelx+1),1), nely+1, nelx+1)
x = reshape(Phi0, nely+1, nelx+1);
for i=1:nely+1
    for j=1:nelx+1
        if x(i,j)<=1e-4 && x(i,j)>=0
            x(i,j)=1e-4;
        end
        if x(i,j)>=-1e-4 && x(i,j)<=0
            x(i,j)=-1e-4;
...
% output the topology, area and numbering of interface nodes
[A0, Arc, den, xy0] = BoundEle(x); % CALL
A0 = A0/(nely*nelx);
den  = max(den, 0.001);
%[den] = Check(nelx,nely,rmin,0,den); % x = 0;
% element area (density), den -- dim(nely,nelx);

%% INITIALIZE
lambda = v*E/(1+v)/(1-2*v);%%%  平面应变问题 %%%
mu = E/2/(1+v);
lambda = 2*lambda*mu/(lambda+2*mu);%%%  平面应力问题 同99行中一致 %%%
angle = 90;
lx = ew; ly = eh;
...
nel = nelx*nely;
[keLambda, keMu, feLambda, feMu] = elementMatVec(dx/2, dy/2, angle);
...
for i = 1:3
    for j = 1:3
        ...
        if i ==1 && j ==1
            dCH11 = 1/cellVolume*(lambda.*sumLambda + mu.*sumMu);
        elseif i ==1 && j ==2
            dCH12 = 1/cellVolume*(lambda.*sumLambda + mu.*sumMu);
        elseif i ==1 && j ==3
            dCH13 = 1/cellVolume*(lambda.*sumLambda + mu.*sumMu);
        elseif i ==2 && j ==2
            dCH22 = 1/cellVolume*(lambda.*sumLambda + mu.*sumMu);
        elseif i ==2 && j ==3
            dCH23 = 1/cellVolume*(lambda.*sumLambda + mu.*sumMu);
        elseif i ==3 && j ==3
            dCH33 = 1/cellVolume*(lambda.*sumLambda + mu.*sumMu);
        end
    end
end
...
dCH11 = E2N(nelx, nely, dCH11);
dCH12 = E2N(nelx, nely, dCH12);
dCH13 = E2N(nelx, nely, dCH13);
dCH22 = E2N(nelx, nely, dCH22);
dCH23 = E2N(nelx, nely, dCH23);
dCH33 = E2N(nelx, nely, dCH33);

dCH11 = reshape(dCH11,NN,1);
dCH12 = reshape(dCH12,NN,1);
dCH13 = reshape(dCH13,NN,1);
dCH22 = reshape(dCH22,NN,1);
dCH23 = reshape(dCH23,NN,1);
dCH33 = reshape(dCH33,NN,1);

% CH = round(CH);
[KE] = StiffnessMatrix(CH,EW,EH);
% KE = reshape(KE,8*8,1);
CH = reshape(CH,3*3,1);


%% COMPUTE ELEMENT STIFFNESS MATRIX AND FORCE VECTOR (NUMERICALLY)
function [keLambda, keMu, feLambda, feMu] = elementMatVec(a, b, angle)
...
for ii=1:length(xx)
    for jj=1:length(yy)
        ...
        % Jacobian
        J = [dNx; dNy]*[-a a a+2*b/tan(angle*pi/180) 2*b/tan(angle*pi/180)-a; ...
            -b -b b b]';
