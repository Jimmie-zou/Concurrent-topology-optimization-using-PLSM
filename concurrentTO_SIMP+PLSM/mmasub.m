...
%  f0val = The value of the objective function f_0 at xval.  目标函数
...
%          of f_0 with respect to x_j (twice). 灵敏度对设计变量的导数
%          Important note: If second derivatives are not available,
%          simply let df0dx2 = 0*df0dx.%%%%%%%%-----二阶导数可以直接设置为0------%%%
%  fval  = Column vector with the values of the constraint functions f_i,
%          calculated at xval.   约束函数的值
...
%          Important note: If second derivatives are not available,
%          simply let dfdx2 = 0*dfdx.%%%%%%%%-----二阶导数可以直接设置为0------%%%
...
zzz = low + albefa*(xval-low);
alfa = max(max(zzz,xmin),xval-move*(xmax-xmin));
zzz = upp - albefa*(upp-xval);
beta = min(min(zzz,xmax),xval+move*(xmax-xmin));
